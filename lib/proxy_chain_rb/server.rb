module ProxyChainRb
  class Server
    ...
    def initialize(instance_id = nil)
      ...
      self.pid_command          =   "pgrep -fo proxy-chain-instance-id-#{self.instance_id}"
    ...
    def identify_pid
      pid_command_output = `#{self.pid_command}`
      self.pid           = (pid_command_output.to_i > 0 ? pid_command_output.to_i : nil)
    end
